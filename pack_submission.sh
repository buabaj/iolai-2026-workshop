...
if command -v huggingface-cli >/dev/null 2>&1; then
  huggingface-cli download "${HUB_MODEL_ID}" --local-dir "${OUT_DIR}"
...
for name in __init__ items prompts parse verify normalize model pipeline; do
...
- Strategy: \`structured_verify_v1\` (\`IOL_STRATEGY\`)
- Explanations: \`IOL_EXPLAIN=1\`
- Quant fallback: \`IOL_LOAD=bnb\`
EOF

echo "packed ${OUT_DIR}"
echo "upload with: hf upload YOURUSER/iolai-2026-qwen25-7b ${OUT_DIR} --repo-type model"
